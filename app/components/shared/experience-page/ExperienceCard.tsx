...
const ExperienceCard: FC<ExperienceCardProps> = ({ data }) => {
  ...
  const modalToggle = () => setModal(!hasModal);
      ...
      <Card>
        <article className="flex flex-col justify-between w-full h-full p-4 md:p-8">
          <div className="flex flex-col">
            <div>
              <img
                src={`/assets/company/${data?.file}`}
                className="grayscale brightness-[5] min-h-min mb-2 hidden"
                alt=""
                loading="lazy"
                width={50}
              />
          ...
          <p
            className="cursor-pointer text-zinc-200 hover:text-zinc-50 lg:block"
            onClick={modalToggle}>
          ...
          <div>
            ...
            <Link href={data.link} target="_blank" rel="noopener noreferrer">
              <h1 className="text-base leading-relaxed font-semibold flex items-center gap-2">
                {data.name}
                <ExternalLink size={20} style={{ paddingBottom: 2 }} />
              </h1>
            </Link>
          </div>
          ...
          <div>
            ...
            <div className="bg-zinc-900 p-4 rounded-xl w-fit">
              <ul className="employment__type">
                {data.employment_type.map((item: Model.Employment) => (
                  <li className="text-sm leading-relaxed">
                    {item.status} ・ {item.location_type} ・{' '}
                    {calculateDuration({ start: item.start, end: item.end })}
                  </li>
                ))}
              </ul>
            </div>
